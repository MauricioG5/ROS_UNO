...
def Nodo_G():								#Declarar función "Nodo_G"
	rospy.init_node('Nodo_G', anonymous=False)			#Inicializar el nodo con su nombre asignado
	rospy.Subscriber('String_D', String, callback)			#Se suscribe al Topic "String_B" de tipo String por la función callback
	pub_float = rospy.Publisher('Char_G', Char, queue_size=10);""" Declara qué Topic va a publicar, el tipo de dato a publicar y el tamaño 										del Buffer """
	...
	while not rospy.is_shutdown():	#Ciclo que dura mientras ROS esté activo
		...
		vect=[None,None,None]
		vect[0] = float(recibido[5:recibido.find("Medio")-2])				#Capturar numéricamente el porcentaje de "Alto"
		vect[1] = float(recibido[recibido.find("Medio")+6:recibido.find("Bajo")-2])	#Capturar numéricamente el porcentaje de "Medio"
		vect[2] = float(recibido[recibido.find("Bajo")+5:len(recibido)-1])		#Capturar numéricamente el porcentaje de "Bajo"
		mayor=None
		i_max=None
		i=0
		for n in vect:
...
if __name__ == '__main__':						#Entrar al main de la función
	try:
		Nodo_G()						#Llamar a la función "Nodo_G"
